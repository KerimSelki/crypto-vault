...
const AuthScreen = ({ onLogin }) => {
  ...
  const getUsers = () => {
    try { return JSON.parse(localStorage.getItem("ip_users") || "{}"); } catch(e) { return {}; }
  };
    ...
    <div style={{minHeight:"100vh",background:"#0B0D15",display:"flex",fontFamily:"'Inter',sans-serif"}}>
      ...
      <div style={{flex:1,background:"linear-gradient(135deg,#1A0D2E 0%,#0B0D15 50%,#1A1508 100%)",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",padding:40,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1,textAlign:"center",maxWidth:400}}>
          ...
          <div style={{fontSize:15,color:"#4A4D65",lineHeight:1.6,marginBottom:32}}>Kripto, BIST, TEFAS ve ABD hisseleri. Gerçek zamanlı fiyatlar, detaylı analizler ve profesyonel portföy yönetimi.</div>
        ...
        <div style={{position:"absolute",bottom:24,fontSize:11,color:"#1E2035"}}>© 2026 InvestPulse — Tüm hakları saklıdır</div>
      </div>
      ...
      <div style={{width:480,display:"flex",alignItems:"center",justifyContent:"center",padding:40,background:"#080A12"}}>
        <div style={{width:"100%",maxWidth:380}}>
          ...
          {mode==="login"&&<>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:"#4A4D65"}}>
                <div onClick={()=>setRememberMe(!rememberMe)} style={{width:18,height:18,borderRadius:4,border:`1px solid ${rememberMe?"#9333EA":"#2A2D45"}`,background:rememberMe?"#9333EA22":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all .2s"}}>
          ...
          {/* FORGOT */}
          {mode==="forgot"&&<>
            <h2 style={{fontSize:24,fontWeight:700,color:"#E8E9ED",marginBottom:4}}>Şifre Kurtarma</h2>
            <p style={{fontSize:13,color:"#4A4D65",marginBottom:28}}>Kayıtlı e-posta adresinizi girin</p>
          ...
          {mode==="register"&&<div style={{textAlign:"center",marginTop:20,fontSize:12,color:"#2A2D45"}}>
